...
function divider(numbersArray) {
  ...
  const evensAndOdds = [evensArray, oddsArray];
  return evensAndOdds;
}
...
function removeItem(itemList, itemToRemove) {
  if (!itemList || !itemToRemove) {
    return []; 
  }
  else 
...
function addItem(itemList, itemToAdd) {
  if (!itemList || !itemToAdd) {
    return [];
  }
